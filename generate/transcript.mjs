		...
		messages: [
			...
			// },
			{
				...
				content: `Create a dialogue for a short-form conversation. The conversation should be between two agents, ${agentA.replace(
					...
					} EXCHANGES, aiming for a concise transcript that would last less than 40 seconds. The person attribute should either be ${agentA} or ${agentB}. The line attribute should be that character's line of dialogue. I also need an asset description under the asset attribute which would be a relevant search query to find an image which should be relevant to the overall topic of the conversation. The asset descriptions shouldn't be vague, but a description of something that you think would be a good image to go along with the conversation. Specificity is key. The JSON format WHICH MUST BE ADHERED TO ALWAYS is as follows: { transcript: { [ {'person': 'the exact value of ${agentA} or ${agentB} depending on who is talking', 'line': 'their line of conversation in the dialog. important to note that if they are mentioning any code specifics even like the word javascript make sure it is spelled in a way that makes sure AI voice overs pronounce it properly in the line attribute! For example: write api as A-P-I to ensure they pronounce the letters correctly!', 'code': \`well formatted valid javascript code content here. always include the output as comment in console.log lines include newline characters and NEVER USE BACKTICKS \"\`\" INSIDE THE CODE CONTENT!\`,'asset': 'relevant image generate query based on the current line'} ] } }`,
			},
			{
				role: 'user',
				...
				content: `generate a video about any specific Impact of A-I in the future of humanity. Current trends in tech companies building robots and more. Both the agents should talk about it in a way they would normally, but extremify their qualities and make the conversation risque if either agents are controversial. Also the conversation should be resolved at the end. Make the second person teach the first person about any inbuilt useful function of javascript. Also, always end with a like and subscribe to learn more code message from both!`,
			},
		],
